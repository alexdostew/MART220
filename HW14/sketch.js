function setup() {
  ...
  function drawShapes(number1, number2) {
    let startX = floor(random(number1-10)) + 200;
    let startY = floor(random(number2));
    console.log(startX);
    // draw shapes
    for (x = startX; x < startX + 70; x++) {
      for (y = startY; y < startY + 70; y++) {
        if (x > startX + 10 && x < startX + 25 && y > startY + 10 && y < startY + 25) {
          ...
          writeColor(img, x, y, 55, 200, 225, floor(random(255)));
        }
      }
    }
  }
  ...
  for (y = 0; y < img.height; y++) {
    for (x = 0; x < img.width; x++) {
      ...
      let blue = random(255);
      let alpha = 255;
      writeColor(img, x, y, red, 200, blue, alpha);
    }
  }
